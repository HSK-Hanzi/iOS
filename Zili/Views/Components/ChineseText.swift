...
struct ChineseText: View {
  ...
  /// Drops everything held about text this run no longer shows. Both call sites lay their runs out
  /// in a `ForEach` keyed by position, so a different entry hands this same view a different string
  /// — and a peek, or a range indexing the old characters, would go on being drawn against the new.
  ...
  /// Looks up the hovered word once the pointer has held still. Cancelled and restarted by every
  /// change of hovered character, so a sweep resolves nothing until it stops — and takes the last
  /// word's peek down as it goes, rather than leaving it anchored where the pointer no longer is.
  private func resolveHover() async {
    if !isPeekingWord(under: hover) { peek = nil }
    guard let hover else { return }
    try? await Task.sleep(for: Self.hoverIntent)
    guard !Task.isCancelled else { return }
    // An ideograph that neither the segmenter nor the dictionary covers has nothing to peek — and
    // the word the pointer has since left is not it.
    guard let range = wordRange(containing: hover.index) else {
      peek = nil
      return
    }
  ...
  /// Whether the peek on screen is already for the word the pointer is on. The one case where a
  /// peek outlives a change of hovered character: crossing between two characters of the same word
  /// should not blink it away and wait out the intent delay to draw the same thing again.
  private func isPeekingWord(under hover: HoverTarget?) -> Bool {
    guard let hover, let peek else { return false }
    return peek.range.contains(hover.index)
  }
  ...
  /// Whether the character sits in the word a peek is showing — tapped, or merely pointed at. The
  /// highlight is what says how much of the run the peek is about, since a peek beside the pointer
  /// names the word but not where it began.
...
extension WordPeek {
  ...
  init(match: ChineseText.Match, script: ChineseScript, romanization: Romanization) {
    self.init(
      ...
      reading: match.lookup.romanization(romanization).map(romanization.spoken),
