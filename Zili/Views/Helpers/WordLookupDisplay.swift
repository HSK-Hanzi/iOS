...
extension WordLookup {
  ...
  /// dictionaries, falling back to the first gloss of any kind (e.g. a monolingual source), and
  /// then to the curated HSK meaning for a syllabus word no loaded dictionary covers.
  var primaryGloss: String? {
    let glosses =
      ...
      .map(\.gloss)
      .filter { !$0.isEmpty }
    return glosses.first { $0.contains { $0.isASCII && $0.isLetter } }
      ?? glosses.first
      ?? hskMeanings.first
  }
  ...
  var definitionSenses: [String] {
    hskMeanings.isEmpty ? bestBilingualGlosses : hskMeanings
  }

  /// The curated meanings for a word in the HSK core syllabus; empty when it isn't in the core,
  /// or when the syllabus records the word without a meaning.
  private var hskMeanings: [String] {
    hskEntries.first?.forms.first?.meanings ?? []
  }
