  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 2) {
      ...
      HStack(spacing: 5) {
        ...
        if let wordCount {
          Text(verbatim: "·")
          Text("\(wordCount, format: .number) words")
        }
      }
      .font(.footnote)
      .foregroundStyle(.secondary)
...
private struct CorpusCount: View {
  ...
  var body: some View {
    switch unit {
      case .entries: Text("\(count, format: .number) entries")
      case .characters: Text("\(count, format: .number) characters")
      case .words: Text("\(count, format: .number) words")
      case .headwords: Text("\(count, format: .number) headwords")
      case .sentences: Text("\(count, format: .number) sentences")
    }
  }
}
