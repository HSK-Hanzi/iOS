...
struct SVGPathTests {
  ...
  func `separators are interchangeable`(_ svg: String) {
    var expected = Path()
  ...
  @Test
  func `signed and decimal coordinates parse`() {
    let path = SVGPath.path(from: "M-1.5,2 L3 4")
    #expect(path.boundingRect == CGRect(x: -1.5, y: 2, width: 4.5, height: 2))
  }

  @Test
  func `an unrecognized command is skipped while the known ones still parse`() {
    let path = SVGPath.path(from: "M0 0 L10 10 W5 5")
    #expect(path.boundingRect == CGRect(x: 0, y: 0, width: 10, height: 10))
  }

  @Test
  func `quadratic and cubic curves parse to a non-empty path`() {
  ...
  func `empty or garbage yields an empty path`(_ svg: String) {
...
struct HanziGeometryTests {
  ...
  @Test
  func `median points map onto a rect with the documented flip and scale`() {
  ...
  @Test
  func `a canvas point round-trips into resolution-independent matching space`() {
