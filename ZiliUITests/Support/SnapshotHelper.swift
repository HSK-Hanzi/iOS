...
// Verbatim fastlane `snapshot` helper (generated by `fastlane snapshot init`), vendored into the
// UI-test target so `GenerateScreenshots` can call `setupSnapshot(app)` / `snapshot(name)`. Keep it
// in step with the upstream version marker at the foot of the file rather than editing it by hand.
